...
async def capture_and_classify(call_id: str, *, client,
    ...
    on_turn: optional async callable(TranscriptTurn) -> None, awaited
    for each turn before classification. Default None (no-op). Observes
    the same SSE stream; does not open a second connection.
    """
    lines: list[str] = []
    try:
